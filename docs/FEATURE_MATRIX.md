...
| Search | 93% | Results and nested folders share the ordinary Miller chain; trailing file preview; Go to Containing Folder selects the actual file | Bounded to four levels and 200 matches; native history and worker cancellation need further work | Expand history and search-scope coverage | Fast complete search with native-quality scope controls |
| Recent columns | 75% | Recent aliases resolve to actual file URIs; previews and Go to Containing Folder verified live | Complete file-operation coverage remains to be tested | Exercise rename, move, and stale recent entries | Full Recent parity |
