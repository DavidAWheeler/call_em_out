...
| Drag and drop | 93% | Multi-file sources, local-move default, remote-copy default, direct folder rows, hover-open, Trash drag-out, and pre-drag selection restoration on cancel | Cancelling a materialized Trash drag can leave its file in the user cache | Restore cancelled staging files to Trash | Native semantics everywhere |
