...
class MyComputerColumnRow(Gtk.Box):
    ...
    def _on_drag_end(self, _source, _drag, delete_data: bool) -> None:
        ...
        if column is not None:
            host = getattr(column, "_mc_host", None)
            restore_committed = getattr(host, "_restore_committed_drag_state", None)
...
class MyComputerColumn(Gtk.ScrolledWindow):
    ...
    def _populate_rows(self, infos: list, dir_counts: dict, generation: int | None = None) -> None:
        ...
        # A monitor refresh commonly reports a temporary download rename even
        # though the visible directory entries are unchanged. Reusing the
        # existing objects in that case keeps GtkListView rows, scroll position,
        # and selection intact, so the window never blanks while Chrome writes.
        old_items = [self._store.get_item(i) for i in range(self._store.get_n_items())]
        same_shape = len(old_items) == len(items) and all(
            old.uri == new.uri
            and old.display_name == new.display_name
            and old.is_dir == new.is_dir
        ...
        if same_shape:
            for old, new in zip(old_items, items):
                ...
                old.mtime = new.mtime
                row = self._row_widgets.get(id(old))
                if row is not None:
                    row.bind(old)
        else:
    ...
    def _on_content_changed(self, _monitor, file, other, event):
        # Writes to an existing file must never rebuild the directory model.
        # Chrome emits many of these while a download grows; the only visible
        # change is the selected preview's size. Keep that update local.
        if event in {
            Gio.FileMonitorEvent.CHANGED,
            Gio.FileMonitorEvent.ATTRIBUTE_CHANGED,
            Gio.FileMonitorEvent.CHANGES_DONE_HINT,
        }:
            host = getattr(self, "_mc_host", None)
            preview = getattr(host, "preview_column", None)
            preview_uri = getattr(preview, "file_uri", None)
            if file is not None and preview_uri:
                try:
                    if file.equal(Gio.File.new_for_uri(preview_uri)):
                        refresh = getattr(preview, "refresh_metadata", None)
                        if callable(refresh):
                            refresh()
        ...
        # Membership events still need an enumeration for creates/deletes and
        # renames, but defer it briefly so the burst generated by a download's
        # temporary file is handled as one transaction.
        membership_events = {
        ...
        self._content_refresh_id = GLib.timeout_add(120, self._reload_after_content_change)

    def _reload_after_content_change(self):
        self._content_refresh_id = 0
        # Preserve the current model while the replacement enumeration runs;
        # _populate_rows() will splice only when names/order actually changed.
        self._refresh_content()
        return GLib.SOURCE_REMOVE

    def _refresh_content(self) -> None:
        """Enumerate this directory without clearing its visible model."""
        if self.folder_uri.startswith("column-search:"):
            return
        self._load_generation += 1
        generation = self._load_generation
...
class MyComputerPreviewColumn(Gtk.Box):
    ...
    def refresh_metadata(self) -> None:
        """Refresh size/timestamps without rebuilding the preview surface.

        Directory monitors report a CHANGED event for every write. Query only
        the small metadata set needed by the details labels so an active
        download can grow in place without restarting text/image decoding or
        flashing the preview.
        """
        if not self.file_uri or self._cancellable.is_cancelled():
            return
        Gio.File.new_for_uri(self.file_uri).query_info_async(
    ...
    def _on_metadata_ready(self, gfile: Gio.File, result: Gio.AsyncResult) -> None:
        try:
        ...
        parts.append(_format_size(info.get_size()))
        self._detail_lbl.set_label(" · ".join(parts))
        self._created_val.set_label(_format_datetime(info.get_attribute_uint64("time::created")))
        self._modified_val.set_label(_format_datetime(info.get_attribute_uint64("time::modified")))
