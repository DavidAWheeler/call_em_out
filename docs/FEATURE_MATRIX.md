...
| Preview content | 94% | Filename-first layout, bordered plain-text/config/code excerpts, images, 1024-tier PDF thumbnails, full paths, and bottom-pinned containing-folder action | PDF rendering still depends on the installed GNOME thumbnailer | Add optional document renderers behind a cheap capability check | Readable previews without opening a second window |
...
| Startup and Home routing | 97% | Startup hands off its optional Computer redirect once; sidebar destinations explicitly re-root and reset horizontal scroll even when Nautilus emits no location change | Still relies on Nautilus's private sidebar row model | Extend the live compatibility probe across supported Nautilus versions | Home and Computer always agree with the selected sidebar row |
